...
zip -r ${NAME}-modules.zip nodejs/node_modules && \
    ...
    aws lambda create-function --function-name ${NAME} --zip-file fileb://${NAME}.zip --handler index.handler --runtime nodejs14.x --role ${ROLE} --timeout 30 --layers ${LAYER_VERSION} --environment Variables="{}" && \
